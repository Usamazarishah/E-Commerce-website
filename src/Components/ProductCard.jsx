...
export default function ProductCard(Props) {
  ...
  //useContext hook product add to cart
  const { addItemToCart,isItemAdded } = useContext(CartContext);
  return (
    <Link
      to={`/product/${Props.id}`}
      className={`w-[calc(50%-8px)] sm:w-[calc(32%)] lg:w-[calc(25%-10px)] xl:w-[calc(25%-28px)] h-auto mt-5 rounded mx-auto ${
    ...
    >
      ...
      <button 
        ...
        className="bg-black w-full h-10 text-white rounded-b-sm"
      >
       {isItemAdded(Props.id) ? `Added (${isItemAdded(Props.id).quantity})` : `Add To Cart`}
      </button>
